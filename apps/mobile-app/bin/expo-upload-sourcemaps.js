...
function groupAssets(assetPaths) {
  ...
  for (const assetPath of assetPaths) {
    ...
    const assetGroupName =
      extname === '.map' ? path.join(parsedPath.dir, parsedPath.name) : path.format(parsedPath);
...
const sentryCliBin =
  getEnvVar(SENTRY_CLI_EXECUTABLE) || require.resolve('@sentry/cli/bin/sentry-cli');
...
if (!outputDir) {
  ...
  console.error(
    'Example: node node_modules/@sentry/react-native/scripts/expo-upload-sourcemaps dist'
  );
  process.exit(1);
}
...
for (const [assetGroupName, assets] of Object.entries(groupedAssets)) {
  ...
  execSync(
    `${windowsCallback}${sentryCliBin} sourcemaps upload ${
      isHermes ? '--debug-id-reference' : ''
    } --release ${process.argv[3]} --dist ${process.argv[4]} ${assets.join(' ')}`,
    {
      env: {
        ...process.env,
...
if (numAssetsUploaded === totalAssets) {
  ...
  console.warn(
    `⚠️  Uploaded ${numAssetsUploaded} of ${totalAssets} bundles and sourcemaps. ${
      numAssetsUploaded === 0
        ? 'Ensure you are running `expo export` with the `--dump-sourcemap` flag.'
        : ''
    }`
  );
}
